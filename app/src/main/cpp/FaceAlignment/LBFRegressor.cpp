...
void  LBFRegressor::WriteRegressor(ofstream& fout){
    ...
    fout<<endl;
    ofstream fout_reg;
    fout_reg.open((modelPath + "regressor.model").c_str(),ios::binary);
...
void LBFRegressor::ReadRegressor(ifstream& fin){
    ...
    ifstream fin_reg;
    fin_reg.open((modelPath + "regressor.model").c_str(),ios::binary);

    for (int i=0; i < global_params.max_numstage; i++ ){
		//cout << "randomforest read" << endl;
